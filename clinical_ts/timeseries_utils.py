__all__ = ['nn_upsample','resample_labels','butter_filter', 'butter_filter_frequency_response', 'apply_butter_filter', 'save_dataset', 'load_dataset',
           ...
           'ChannelFilter', 'Transform', 'TupleTransform', 'SequenceToSampleLabelTransform', 'aggregate_predictions']
...
def npys_to_memmap_batched(npys, target_filename, max_len=0, delete_npys=True, batch_length=900000):
    ...
    for idx,npy in tqdm(list(enumerate(npys))):
        ...
        channels = np.prod(data.shape[1:]) if len(data.shape)>1 else 1
    ...
    def __init__(self, df, output_size, chunk_length, min_chunk_length, memmap_filename=None, memmap_label_filename=None, npy_data=None, random_crop=True, data_folder=None, copies=0, col_data="data", col_lbl="label", cols_static=None, stride=None, start_idx=0, annotation=False, transforms=None, sample_items_per_record=1, fs_annotation_over_fs_data=1):
        ...
        if(memmap_filename is not None):
            ...
            if(annotation):
                ...
                self.memmap_start_label = memmap_meta_label["start"]
                self.memmap_shape_label = memmap_meta_label["shape"]
                self.memmap_length_label = memmap_meta_label["length"]
                self.memmap_file_idx_label = memmap_meta_label["file_idx"]
